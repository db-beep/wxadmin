<template>
  <div class="x-header x-space-between">
    <div class="x-row">
...
import institutionPersonal from '../../views/institution/Edit'
...
const personal = async ()=>{
  ...
  if(userInfo.value.role === 'institution'){
    const op = Dialog.open(institutionPersonal, `个人中心`)
    op.mounted(c => {
      c.render(id,()=>{Msg.success("个人信息已更新")})
    })
    op.confirm(async (c) => {
