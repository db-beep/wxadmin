<template>
  ...
  <el-row class="search-row">
    <div class="search-item">
      ...
      <div style="width: 150px">
        <el-input clearable placeholder="请输入非遗文化标题" v-model="searchForm.showtitle" maxlength="25"/>
      </div>
    </div>
    ...
    <div class="search-item">
      <span>非遗文化标签</span>
      <div style="width: 150px">
        <x-select-table :header="[{label:'非遗文化类型',field:'name',width:'100%'}]"
          v-model="searchForm.labels"
          :queryParams="[{name:'name',label:'非遗文化类型'}]"
    ...
    <button class="x-btn x-btn-success x-btn-m" v-if="user && ( user.role.toLowerCase() == 'institution'   )" @click="onAdd">新增科普</button>
  ...
  <el-table :data="pager.records" style="width: 100%">
    ...
    <el-table-column label="发布机构">
      <template #default="scope">
        <el-button link v-if="scope.row.fabjgFrn" type="primary" text bg @click="fabjgDetail(scope.row.fabjg)">{{ scope.row.fabjgFrn.name }}</el-button>
      </template>
    </el-table-column>
    <el-table-column label="非遗文化标签">
      <template #default="scope">
        <el-tag type="success" v-if="scope.row.labelsLabels" v-for="i in scope.row.labelsLabels">{{i.name}}</el-tag>
      </template>
    </el-table-column>
    <el-table-column label="非遗文化简述">
      <template #default="scope">
...
<script setup name="intangibleCultural">
    let conf = codeying
    ...
    const loadThisPage = async () => {
        ...
        let {data,message} = await Http.get(`/intangibleCultural/list`, params);
        console.log("查询条件")
        console.log(params)
        console.log("查询结果")
        console.log(data)
    ...
    const ups = async (id, msg, field, value) => {
      ...
      op.then(async () => {
        ...
        let {success, message} = await Http.post(`/intangibleCultural/save`, upsForm);
        if (!success) {
          Msg.error(message);
        }else{
          Msg.success(message);
    ...
    const onDelete = async (id) => {
        ...
        op.then(async () => {
            let {success, message} = await Http.post(`/intangibleCultural/delete?id=` + id);
            if (!success) {
                Msg.error(message);
            } else {
                Msg.success(message);
