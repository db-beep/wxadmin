    ...
    <div class="wlist-main-content">
      <div class="wlist-m-body">
        <div class="wlist-content-title">
          <h2>评论列表</h2>
          <div class="wlist-count">共 {{pager.total}} 条评论</div>
        </div>
        <!-- 搜索 -->
        <div class="wlist-search-filter-section">
          ...
          <div class="wlist-search-form">
            ...
            <div class="wlist-form-group">
              <label >用户名</label>
              <input maxlength="255" type="text" v-model="form.username" placeholder="请输入用户名" >
            </div>
            <div class="wlist-form-group">
              <label >用户角色</label>
              <input maxlength="255" type="text" v-model="form.rolech" placeholder="请输入用户角色" >
            </div>
            <div class="wlist-form-group">
              <label >内容</label>
              <input maxlength="255" type="text" v-model="form.content" placeholder="请输入内容" >
            </div>
            <div class="wlist-form-group">
              <label class="">发布时间</label>
              <input autocomplete="off" type="date" name="createtimeL" v-model="form.createtimeL" placeholder="从此开始" />
            </div>
            <div class="wlist-form-group">
              <label class="">~</label>
              <input autocomplete="off" type="date" name="createtimeR" v-model="form.createtimeR" placeholder="到此结束" />
            </div>
            <div class="wlist-form-group">
...
<script setup>
  ...
  onMounted(async () => {
    user.value = Cache.getUser()
        await loadThisPage()
    webLayout.value.activeNav("userComment");
  })
  ...
  const loadThisPage = async () => {
    ...
    let {data,message} = await Http.get("/webu/userComment/list", params);
    if (data) {
      pager.value = data
    }else {
      Msg.error(message);
  ...
  import DetailPage from "../userComment/Detail"
  //详情页
  ...
  import EditPage from "../userComment/Edit"
  //编辑页
  ...
  const ups = async (id, msg, field, value) => {
    ...
    op.then(async () => {
      ...
      let {success, message} = await Http.post(`/webu/userComment/save`, upsForm);
      if (!success) {
        Msg.error(message);
      }else{
        Msg.success(message);
  ...
  import AddPage from "../userComment/Add"
  //新增页
  const onAdd = async () => {
  ...
  const onDelete = async (id) => {
    ...
    op.then(async () => {
      let {success, message} = await Http.post(`/webu/userComment/delete?id=` + id);
      if (!success) {
        Msg.error(message);
      } else {
        Msg.success(message);
