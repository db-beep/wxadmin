    ...
    <div class="wlist-main-content">
      <div class="wlist-m-body">
        <div class="wlist-content-title">
          <h2>公告列表</h2>
          <div class="wlist-count">共 {{pager.total}} 条公告</div>
        </div>
        <!-- 搜索 -->
        <div class="wlist-search-filter-section">
          ...
          <div class="wlist-search-form">
            ...
            <div class="wlist-form-group">
              <label >标题</label>
              <input maxlength="255" type="text" v-model="form.title" placeholder="请输入标题" >
            </div>
            <div class="wlist-form-group">
...
<script setup>
  ...
  onMounted(async () => {
    user.value = Cache.getUser()
        await loadThisPage()
    webLayout.value.activeNav("notice");
  })
  ...
  const loadThisPage = async () => {
    ...
    let {data,message} = await Http.get("/webu/notice/list", params);
    if (data) {
      pager.value = data
    }else {
      Msg.error(message);
  ...
  import DetailPage from "../notice/Detail"
  //详情页
  ...
  import EditPage from "../notice/Edit"
  //编辑页
  ...
  const ups = async (id, msg, field, value) => {
    ...
    op.then(async () => {
      ...
      let {success, message} = await Http.post(`/webu/notice/save`, upsForm);
      if (!success) {
        Msg.error(message);
      }else{
        Msg.success(message);
  ...
  import AddPage from "../notice/Add"
  //新增页
  const onAdd = async () => {
  ...
  const onDelete = async (id) => {
    ...
    op.then(async () => {
      let {success, message} = await Http.post(`/webu/notice/delete?id=` + id);
      if (!success) {
        Msg.error(message);
      } else {
        Msg.success(message);
