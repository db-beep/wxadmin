    ...
    <div class="wlist-main-content">
      <div class="wlist-m-body">
        <div class="wlist-content-title">
          <h2>非遗文化科普列表</h2>
          <div class="wlist-count">共 {{pager.total}} 条非遗文化科普</div>
        </div>
        <!-- 搜索 -->
        <div class="wlist-search-filter-section">
          ...
          <div class="wlist-search-form">
            ...
            <div class="wlist-tag-filters">
              <input type="radio" name="labels" id="all" value="" class="wlist-radio-tag" checked>
              <label for="all">全部</label>
              <template v-for="i in labelsFrnList">
                <input v-model="form.labels" type="radio" name="labels" :id="i.id" :value="i.id" class="wlist-radio-tag">
            ...
            <div class="wlist-form-group" v-if="!(user && user.role == 'institution')">
              ...
              <option v-for="i in fabjgFrnList" :value="i.id">{{ i.name }}</option>
              </select>
            </div>
            <div class="wlist-form-group">
            ...
            <a class="wlist-form-btn wlist-form-btn-wlist-success" v-if="user && ( user.role.toLowerCase() == 'institution'   )" @click="onAdd"><i class="fas fa-plus-square"></i> 新增科普</a>
          </div>
        </div>
        ...
        <div class="wlist-content-table-container">
          ...
          <td>{{ i.showtitle}}</td>
          <td>
          <el-button link v-if="i.fabjgFrn" type="primary" text bg @click="fabjgDetail(i.fabjg)">{{ i.fabjgFrn?.name }}</el-button>
          </td>
          <td>
          <a v-for="label in i.labelsLabels" href="#"><span class="label label-primary">{{ label.name }}</span></a>
          </td>
          <td>{{ i.showdesc}}</td>
          <td>{{ i.publishtime}}</td>
          <td>
          ...
          <a v-if="user &&( user.role.toLowerCase() == 'institution'  )" @click="onEdit(i.id)" class="wlist-form-btn wlist-form-btn-wlist-xs wlist-form-btn-wlist-primary"><i class="fas fa-edit"></i> 修改</a>
          <a v-if="user &&(  user.role.toLowerCase() == 'institution'  )" @click="onDelete(i.id)" class="wlist-form-btn wlist-form-btn-wlist-xs wlist-form-btn-wlist-danger"><i class="far fa-trash-alt"></i> 删除</a>
...
<script setup>
  ...
  const pager = ref({current: 1, size: 20,records:[]})
        ...
        const fabjgFrnList = ref([]);
        const labelsFrnList = ref([]);
    
  onMounted(async () => {
    user.value = Cache.getUser()
          let fabjgFrnResp = await Http.get("/webu/institution/list/select");
        fabjgFrnList.value = fabjgFrnResp.data
          let labelsFrnResp = await Http.get("/webu/cultureType/list/select");
        labelsFrnList.value = labelsFrnResp.data
        await loadThisPage()
  ...
  const loadThisPage = async () => {
    ...
    let {data,message} = await Http.get("/webu/intangibleCultural/list", params);
    if (data) {
      pager.value = data
    }else {
      Msg.error(message);
  ...
  import DetailPage from "../intangibleCultural/Detail"
  //详情页
  ...
  import EditPage from "../intangibleCultural/Edit"
  //编辑页
  ...
  const ups = async (id, msg, field, value) => {
    ...
    op.then(async () => {
      ...
      let {success, message} = await Http.post(`/webu/intangibleCultural/save`, upsForm);
      if (!success) {
        Msg.error(message);
      }else{
        Msg.success(message);
  ...
  import AddPage from "../intangibleCultural/Add"
  //新增页
  const onAdd = async () => {
    const op = Dialog.open(AddPage, `新增科普`).setConfirmText('确认')
    op.mounted(c => {
  ...
  const onDelete = async (id) => {
    ...
    op.then(async () => {
      let {success, message} = await Http.post(`/webu/intangibleCultural/delete?id=` + id);
      if (!success) {
        Msg.error(message);
      } else {
        Msg.success(message);
