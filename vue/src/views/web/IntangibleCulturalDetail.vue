    ...
    <div class="m-body main-detail">
      ...
      <div class="detail-head">
        ...
        <div class="detail-head-box2">
          ...
          <div>
            <span v-for="label in detail.labelsLabels" class="label label-primary">{{ label.name }}</span>
          </div>
      ...
      <div class="detail-body">
        ...
        <div class="detail-body2">
          <h2>内容详情</h2>
          <!-- 产品详情字段 -->
          <template v-if="detail.fabjgFrn">
            <h3>发布机构</h3>
            <p>{{ detail.fabjgFrn.name}}</p>
          </template>
          <h3>科普内容详情</h3>
...
<script setup>
  ...
  let entityName = `intangibleCultural`//实体类型
  let detail = ref({})//数据
  ...
  const loadData = async (id) => {
    Msg.loading("正在前往...")
    let {data} = await Http.get("/webu/intangibleCultural/detail?id=" + id);
