    ...
    <div class="wlist-main-content">
      <div class="wlist-m-body">
        <div class="wlist-content-title">
          <h2>非遗传承活动列表</h2>
          <div class="wlist-count">共 {{pager.total}} 条非遗传承活动</div>
        </div>
        <!-- 搜索 -->
        <div class="wlist-search-filter-section">
          ...
          <div class="wlist-search-form">
            ...
            <div class="wlist-form-group">
              <label >活动名称</label>
              <input maxlength="18" type="text" v-model="form.name" placeholder="请输入活动名称" >
            </div>
            <div class="wlist-form-group" v-if="!(user && user.role == 'institution')">
              <label for="zhubf">主办机构：</label>
              <select id="zhubf" v-model="form.zhubf">
              <option value="" selected>全部</option>
              <option v-for="i in zhubfFrnList" :value="i.id">{{ i.name }}</option>
              </select>
            </div>
            ...
            <a class="wlist-form-btn wlist-form-btn-wlist-success" v-if="user && ( user.role.toLowerCase() == 'institution'   )" @click="onAdd"><i class="fas fa-plus-square"></i> 发布非遗传承活动</a>
          </div>
        </div>
        ...
        <div class="wlist-content-table-container">
          ...
          <td>{{ i.name}}</td>
          <td>
          <el-button link v-if="i.zhubfFrn" type="primary" text bg @click="zhubfDetail(i.zhubf)">{{ i.zhubfFrn?.name }}</el-button>
          </td>
          <td>{{ i.content}}</td>
          <td>{{ i.baomsm}}</td>
          <td>{{ i.starttime}}</td>
          ...
          <a class="wlist-form-btn wlist-form-btn-wlist-xs wlist-form-btn-wlist-primary" v-if="user && ( user.role.toLowerCase() == 'user'   )" @click="onAddBaomByAcid(i.id)"><i class="fas fa-plus-square"></i>立即报名</a>
...
<script setup>
  ...
  const pager = ref({current: 1, size: 20,records:[]})
        ...
        const zhubfFrnList = ref([]);
    
  onMounted(async () => {
    user.value = Cache.getUser()
          let zhubfFrnResp = await Http.get("/webu/institution/list/select");
        zhubfFrnList.value = zhubfFrnResp.data
        await loadThisPage()
    webLayout.value.activeNav("culActivi");
  })
  ...
  const loadThisPage = async () => {
    ...
    let {data,message} = await Http.get("/webu/culActivi/list", params);
    if (data) {
      pager.value = data
    }else {
      Msg.error(message);
  ...
  import DetailPage from "../culActivi/Detail"
  //详情页
  ...
  import EditPage from "../culActivi/Edit"
  //编辑页
  ...
  const ups = async (id, msg, field, value) => {
    ...
    op.then(async () => {
      ...
      let {success, message} = await Http.post(`/webu/culActivi/save`, upsForm);
      if (!success) {
        Msg.error(message);
      }else{
        Msg.success(message);
  ...
  import AddPage from "../culActivi/Add"
  //新增页
  const onAdd = async () => {
  ...
  const onDelete = async (id) => {
    ...
    op.then(async () => {
      let {success, message} = await Http.post(`/webu/culActivi/delete?id=` + id);
      if (!success) {
        Msg.error(message);
      } else {
        Msg.success(message);
