    ...
    <div class="wlist-main-content">
      <div class="wlist-m-body">
        <div class="wlist-content-title">
          <h2>用户列表</h2>
          <div class="wlist-count">共 {{pager.total}} 条用户</div>
        </div>
        <!-- 搜索 -->
        <div class="wlist-search-filter-section">
          ...
          <div class="wlist-search-form">
            ...
            <div class="wlist-form-group">
              <label >姓名</label>
              <input maxlength="18" type="text" v-model="form.name" placeholder="请输入姓名" >
            </div>
            <div class="wlist-form-group">
        ...
        <div class="wlist-content-table-container">
          ...
          <a v-if="user &&( user.role.toLowerCase() == 'admin'  )" @click="onEdit(i.id)" class="wlist-form-btn wlist-form-btn-wlist-xs wlist-form-btn-wlist-primary"><i class="fas fa-edit"></i> 编辑</a>
...
<script setup>
  ...
  onMounted(async () => {
    user.value = Cache.getUser()
        ...
        await loadThisPage()
    webLayout.value.activeNav("user");
  })
  ...
  const loadThisPage = async () => {
    ...
    let {data,message} = await Http.get("/webu/user/list", params);
    if (data) {
      pager.value = data
    }else {
      Msg.error(message);
  ...
  import DetailPage from "../user/Detail"
  //详情页
  ...
  import EditPage from "../user/Edit"
  //编辑页
  ...
  const ups = async (id, msg, field, value) => {
    ...
    op.then(async () => {
      ...
      let {success, message} = await Http.post(`/webu/user/save`, upsForm);
      if (!success) {
        Msg.error(message);
      }else{
        Msg.success(message);
  ...
  const onDelete = async (id) => {
    ...
    op.then(async () => {
      let {success, message} = await Http.post(`/webu/user/delete?id=` + id);
      if (!success) {
        Msg.error(message);
      } else {
        Msg.success(message);
