<template>
  <el-form label-position="left" label-width="100px" ref="formRef" :model="form" style="max-width: 600px" :rules="rules">
    <el-form-item label="非遗类型名" prop="name">
...
<script setup>
    ...
    const rules = reactive({
        name:{required: true, message: "非遗类型名必填", trigger: "blur"},
    })


    const render = async (id,loadThisPage) => {
        Msg.loading("加载中..")
        let {data} = await Http.get(`/cultureType/detail`, {id: id});
        form.value = data
    ...
    const submit = async () => {
        ...
        await formRef.value.validate(async (isValid, invalidFields) => {
            ...
            let {success,message} = await Http.post(`/cultureType/save`, form.value);
            if(! success) {
                Msg.error(message);
                return;
            }
