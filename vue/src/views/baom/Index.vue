<template>
  ...
  <el-row class="search-row">
    ...
    <div v-if="user.role!=='culActivi'" class="search-item">
      <span>报名活动</span>
      <div style="width: 150px">
        <x-select-table :header="[{label:'非遗传承活动',field:'name',width:'100%'}]"
          v-model="searchForm.acid"
    ...
    <div class="search-item">
      <span>报名备注</span>
      <div style="width: 150px">
        <el-input clearable placeholder="请输入报名备注" v-model="searchForm.remark" maxlength="255"/>
      </div>
    </div>
    <div class="search-item">
      <span>审核</span>
    ...
    <button class="x-btn x-btn-success x-btn-m" v-if="user && ( user.role.toLowerCase() == 'user'   )" @click="onAdd">立即报名</button>
  ...
  <el-table :data="pager.records" style="width: 100%">
    ...
    <el-table-column label="报名活动">
      <template #default="scope">
        <el-button link v-if="scope.row.acidFrn" type="primary" text bg @click="acidDetail(scope.row.acid)">{{ scope.row.acidFrn.name }}</el-button>
      </template>
    </el-table-column>
    <el-table-column label="非遗机构">
      <template #default="scope">
        <el-button link v-if="scope.row.jigFrn" type="primary" text bg @click="jigDetail(scope.row.jig)">{{ scope.row.jigFrn.name }}</el-button>
      </template>
    </el-table-column>
    <el-table-column label="报名备注">
      <template #default="scope">
        <span >{{scope.row.remark}}</span>
      </template>
    </el-table-column>
    <el-table-column sortable label="报名时间">
      <template #default="scope">
    ...
    <el-table-column fixed="right" label="操作">
      <template #default="scope">
        ...
        <button class="x-btn x-btn-warn" v-if=" user.role.toLowerCase() == 'institution'  " @click="onEdit(scope.row.id)">处理报名</button>
        <button class="x-btn x-btn-error" v-if="  user.role.toLowerCase() == 'user'  " @click="onDelete(scope.row.id)">取消报名</button>
...
<script setup name="baom">
    ...
    const loadThisPage = async () => {
        ...
        let {data,message} = await Http.get(`/baom/list`, params);
        console.log("查询条件")
        console.log(params)
        console.log("查询结果")
        console.log(data)
    ...
    const ups = async (id, msg, field, value) => {
      ...
      op.then(async () => {
        ...
        let {success, message} = await Http.post(`/baom/save`, upsForm);
        if (!success) {
          Msg.error(message);
        }else{
          Msg.success(message);
    ...
    const onDelete = async (id) => {
        ...
        op.then(async () => {
            let {success, message} = await Http.post(`/baom/delete?id=` + id);
            if (!success) {
                Msg.error(message);
            } else {
                Msg.success(message);
