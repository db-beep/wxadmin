<template>
  <el-form label-position="left" label-width="100px" ref="formRef" :model="form" style="max-width: 600px" :rules="rules">
    ...
    <el-form-item label="报名活动" prop="acid">
      <x-select-table
        :header="[{label:'非遗传承活动',field:'name',width:'100%'}]"
        v-model="form.acid"
        :queryParams="[{name:'name',label:'非遗传承活动'}/*,{name:'state',label:'状态',defaultValue:'上架',type:'hide'}*/]"
        api="/culActivi/list"
        labelField="name"
        valueField="id"
      ></x-select-table>
    ...
    <el-form-item label="报名备注" prop="remark">
      <el-input maxlength="255" v-model="form.remark" />
    </el-form-item>
  </el-form>
</template>
<script setup>
