<template>
  ...
  <el-row class="search-row">
    ...
    <div class="search-item">
      <span>非遗机构名称</span>
      <div style="width: 150px">
        <el-input clearable placeholder="请输入非遗机构名称" v-model="searchForm.name" maxlength="18"/>
      </div>
    </div>
    
    <button class="x-btn x-btn-m" type="primary" @click="loadThisPage">查询</button>
    <button class="x-btn x-btn-success x-btn-m" v-if="user && ( user.role.toLowerCase() == 'admin'   )" @click="onAdd">新增</button>
  ...
  <el-table :data="pager.records" style="width: 100%">
    ...
    <el-table-column label="非遗机构名称">
      <template #default="scope">
        <span >{{scope.row.name}}</span>
      </template>
    </el-table-column>
    <el-table-column label="机构头像">
      <template #default="scope">
...
<script setup name="institution">
    ...
    const loadThisPage = async () => {
        ...
        let {data,message} = await Http.get(`/institution/list`, params);
        console.log("查询条件")
        console.log(params)
        console.log("查询结果")
        console.log(data)
    ...
    const ups = async (id, msg, field, value) => {
      ...
      op.then(async () => {
        ...
        let {success, message} = await Http.post(`/institution/save`, upsForm);
        if (!success) {
          Msg.error(message);
        }else{
          Msg.success(message);
    ...
    const onDelete = async (id) => {
        ...
        op.then(async () => {
            let {success, message} = await Http.post(`/institution/delete?id=` + id);
            if (!success) {
                Msg.error(message);
            } else {
                Msg.success(message);
